...
export async function generateMetadata({ params }: PageProps) {
  ...
  const db = getDb();
  const [player] = await db.select({ fullName: players.fullName }).from(players).where(eq(players.id, id)).limit(1);
  return { title: player ? `${player.fullName} — BOC Sports Profile` : "BOC Sports Profile" };
}
  ...
  return (
    <div className="report-root">
      <style>{`
        ...
        .report-root .header { display: flex; align-items: center; justify-content: space-between; padding-bottom: 20px; border-bottom: 2px solid #A05AFF; margin-bottom: 24px; }
        ...
        .report-root h1 { font-size: 22px; font-weight: 700; margin: 0 0 4px; }
        ...
        .report-root h2 { font-size: 14px; font-weight: 700; margin: 0 0 12px; color: #A05AFF; }
        ...
        .report-root td { padding: 8px 10px; border-bottom: 1px solid #F5F0FF; }
          ...
          <tbody>
            {history.map((r) => (
              <tr key={r.resultId}>
                ...
                <td style={{ color: "#6B6B7B" }}>{EVENT_TYPE_LABELS[r.eventType as EventType]}{r.discipline ? ` · ${r.discipline}` : ""}</td>
                ...
                <td style={{ fontFamily: "monospace", fontSize: 12 }}>{r.performance ?? "—"}</td>
                <td>
