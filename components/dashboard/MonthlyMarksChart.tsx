...
interface Props {
  data: { month: string; marks: number }[];
}
...
export default function MonthlyMarksChart({ data }: Props) {
  if (data.length === 0 || data.every(d => d.marks === 0)) {
    return (
      <div className="h-48 flex flex-col items-center justify-center gap-2 text-text-grey">
        ...
        <p className="text-sm">No results recorded this year</p>
      </div>
    );
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={192}>
      <BarChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }} barCategoryGap="35%">
        ...
        <XAxis dataKey="month" tick={{ fontSize: 11, fill: "#9B9BAA", fontWeight: 500 }} axisLine={false} tickLine={false} />
        <YAxis tick={{ fontSize: 11, fill: "#9B9BAA" }} axisLine={false} tickLine={false} allowDecimals={false} />
        <Tooltip content={<CustomTooltip />} cursor={{ fill: "#F5F0FF", radius: 8 }} />
        <Bar dataKey="marks" radius={[6, 6, 0, 0]}>
          {data.map((entry) => (
            <Cell key={entry.month} fill={entry.marks === maxMarks ? "#A05AFF" : "#D4BBFF"} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
